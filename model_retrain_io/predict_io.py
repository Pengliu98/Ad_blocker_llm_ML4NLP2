...
def extract_ad_spans(text, model, tokenizer, device, threshold=0.5):
    """
    Run sliding-window inference over `text` and return sentence-level
    ad spans where the averaged token ad-probability exceeds `threshold`.
    """
    inputs = tokenizer(
        text,
        return_tensors="pt",
    ...
    # Filter out very short spurious spans (< 20 chars) — fixes false positives
    raw_spans = [s for s in raw_spans if s[1] - s[0] >= 20]

    # Merge nearby raw spans, then split into sentence-level spans
